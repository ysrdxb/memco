        ...
        <div class="row align-items-end">
            <div class="col-lg-4">
                <div class="page-header-title">
                    ...
                    <div class="d-inline">
                        <h5>{{ __('Material Requisition') }}</h5>
                        <span>{{ __('View, delete, and update Material Requisition') }}</span>
                    </div>
                </div>
            </div>
            <div class="col-lg-8 text-right">
                <a class="btn btn-primary mr-4" href="{{ route('transfer.import.create') }}"><i class="fas fa-download"></i> Import</a>			    
                <a class="btn btn-danger" href="{{ route('transfer.create') }}"><i class="ik ik-plus"></i> Create New</a>	
...
<script>
function updateSwitchValue(route) {
    $.ajax({
        url: route,
        method: 'POST',
        data: {
        ...
        error: function(xhr, status, error) {
            Swal.fire({
                icon: 'error',
                title: 'Error!',
                text: 'Failed to update purchase status. Please try again.'
            });
        }
    });
}
$(document).ready(function() {
        ...
        var dTable = $('#_dataTable').DataTable({
            ...
            ajax: {
                url: '{{ route("transfer.getStatusList", $projectId) }}',
                type: "post"
            },
            columns: [
                ...
                { data: 'date', name: 'date' },
                { data: 'lpo', name: 'lpo', orderable: false, searchable: false },
                { data: 'mtv', name: 'mtv', orderable: false, searchable: false },
                { data: 'do', name: 'do', orderable: false, searchable: false },
